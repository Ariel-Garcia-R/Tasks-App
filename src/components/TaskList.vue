...
<template>
 ...
 <div class="fixed bottom-3 rounded h-10 w-2/3 max-w-72 bg-[#0D55CF] left-0 right-0 mr-auto ml-auto flex justify-evenly">
    <button :class="{'bg-blue-800': selection === 'all'}" class="w-[32%] text-white transition-all duration-300 rounded my-1" @click="updateSelectedTasks('all')" >{{$t('allTasks')}}</button>
    <button :class="{'bg-blue-800': selection === 'pending'}" class="w-[32%] text-white transition-all duration-300 rounded my-1" @click="updateSelectedTasks('pending')" >{{ $t('pendingTasks') }}</button>
    <button :class="{'bg-blue-800': selection === 'done'}" class="w-[32%] text-white transition-all duration-300 rounded my-1" @click="updateSelectedTasks('done')" >{{ $t('doneTasks') }}</button>
 </div>
</template>
